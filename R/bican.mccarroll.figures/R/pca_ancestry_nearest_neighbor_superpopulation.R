...
estimate_donor_superpopulation <- function(
    ...
    if (!is.null(bican_donors_file)) {
        bican_donors <- read.table(bican_donors_file)$V1
...
.gather_ancestry_data <- function(
    ...
    kgp_donors <- read.table(kgp_donors_file)

    kgp_donor_ancestry <- read.table(
        kgp_ancestry_file,
        header = TRUE
    )

    pca_list <- lapply(pca_files, read.table)
