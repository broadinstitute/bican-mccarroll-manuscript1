...
run_cell_type_specific_eqtl_tests <- function(
  ...
  if (!is.null(outPDF)) {
    ...
    write_eqtl_summary_pdf(
      results, eqtl_data,
      outfile = outPDF, show_interaction = T,
...
.prepare_testing_data <- function(eqtl_data,
                                  covariate_mapping,
                                  force_factor_covariates = NULL) {
  # Make R CMD CHECK happy
  .SD <- NULL
...
plot_gene_tpm_by_group <- function(geneName,
                                   eqtl_data,
                                   show_title = FALSE) {
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    stop("Package 'ggplot2' is required.", call. = FALSE)
...
write_eqtl_summary_pdf <- function(results,
                                   eqtl_data,
                                   show_interaction = FALSE,
                                   outfile,
                                   width = 11,
