...
compare_all_eQTL_runs <- function(data_dir, outDir = NULL, filter_levels = c(0, 1, 2, 3, 4), fdr_threshold = 0.05,
  ...
  for (i in 1:(length(filter_levels) - 1)) {
    ...
    df <- compare_eqtl_runs_ctr(baseline_data_dir, comparison_data_dir,
  ...
  p11 <- plot_reduction_vs_initial(df,
    cluster_df = z$clusters, baseline_name = "LEVEL_0",
    comparison_name = "LEVEL_3", y_col = "yield", x_col = "n_union_egenes", , point_size = 3
  )

  p12 <- plot_reduction_by_parent_type(
    df,
    baseline_name = "LEVEL_0", comparison_name = "LEVEL_3", x_col = "n_union_egenes", y_col = "yield", , point_size = 3
  )
  ...
  p21 <- plot_reduction_vs_initial(df,
    cluster_df = z$clusters, baseline_name = "LEVEL_0",
...
compare_eqtl_runs <- function(cell_type = "MSN", region = "CaH", baseline_name, comparison_name,
  ...
  final_p1 <- add_supertitle_cowplot(
    p = p1,
  ...
  final_p2 <- add_supertitle_cowplot(
    p = p2,
...
compare_index_eqtls <- function(index_dt, index_comparison_dt, baseline_name, comparison_name, fdr_threshold = 0.05) {
  ...
  z <- plot_gene_venn(index_dt, index_comparison_dt,
    text_size = 6, title_size = 16,
  ...
  z <- plot_gene_venn(eQTLs_base, eQTLs_comp,
    text_size = 6, title_size = 16,
    baseline_name = baseline_name, comparison_name = comparison_name,
  ...
  stats_df <- make_compare_index_eqtls_stats_df(
    stats_genes_tested,
    stats_eGenes,
    effect_size_stats,
    qval_stats
...
test_eqtl_pval_inflation <- function(eqtl_dataframe,
  ...
  if (!is.null(tail_min_p)) {
    stopifnot(
      is.numeric(tail_min_p), length(tail_min_p) == 1,
...
get_or_build_augmented_indices_for_sign <- function(index_file,
  ...
  utils::write.table(augmented$index_dt,
    file = paths$index_path,
    sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE
  )

  utils::write.table(augmented$index_comparison_dt,
    file = paths$comparison_path,
    sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE
  )
