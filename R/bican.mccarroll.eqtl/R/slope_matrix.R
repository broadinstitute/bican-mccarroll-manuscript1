...
# egene_union_pairs_path="/broad/bican_um1_mccarroll/RNAseq/analysis/CAP_freeze_3_analysis/eqtls/script_output/LEVEL_3/egene_union_pairs_neuron_evo_qval_0.05.tsv"
# output_path="/broad/bican_um1_mccarroll/RNAseq/analysis/CAP_freeze_3_analysis/eqtls/script_output/LEVEL_3/slope_matrix_neuron_evo_qval_0.05.tsv"
# bican.mccarroll.eqtl::get_slope_matrix(eqtl_dir, region_cell_type_path, egene_union_pairs_path, output_path)


#' Build a slope matrix for eGene-variant pairs across cell types and regions
#'
#' For each cell type / region in the region-cell-type table, reads the
#' all-pairs tensorQTL output and extracts slope values for the eGene-variant
#' pairs provided in \code{egene_union_pairs_path}.  The result is a matrix
#' with one row per eGene-variant pair and one column per cell type / region.
#'
...
#'   \code{variant_id}, and one slope column per cell type / region
#'   (named \code{<cell_type>__<region>}).
#'
#' @export
#' @importFrom data.table fread fwrite
#' @importFrom logger log_info log_warn
get_slope_matrix <- function(eqtl_dir,
    ...
    for (i in seq_len(nrow(region_cell_type_dt))) {
        ...
        # Read all-pairs slopes and join to eGene union
        ...
        logger::log_info("  Matched {sum(!is.na(slope_dt[[col_name]]))} of {nrow(slope_dt)} pairs")
    }

    slope_dt[, pair_key := NULL]

    if (!is.null(output_path)) {
        data.table::fwrite(slope_dt, output_path, sep = "\t")
