...
#'
#' @return A \code{data.table} with one row per test x cell type x region,
...
#' @param display_numbers Logical scalar. If TRUE, display scaled numeric values
#'   in the heatmap cells.
#' @param lower_percentile Numeric scalar specifying the lower percentile used
#'   as the zero point for displayed values.
#' @param upper_percentile Numeric scalar specifying the upper percentile used
#'   as the one point for displayed values.
#' @param n_digits Non-negative integer specifying the number of decimal places
#'   shown for displayed values.
#'
#' @return A ggplot object.
#'
...
trade_heatmap <- function(trade_results,
                          ...
                          n_digits = 2) {
  # Make R CMD CHECK Happy
  cell_type <- region_plot <- value <- region <- scaled_value <-
    display_label <- display_color <- NULL
  ...
  if (is.null(region_order)) {
    region_order <- setdiff(colnames(wide), "cell_type")
  }
  ...
  long[, scaled_value := .scale_to_percentile_range(
    value,
  ...
  if (display_numbers) {
    long[, display_label := ifelse(
      is.na(scaled_value),
      NA_character_,
      formatC(scaled_value, format = "f", digits = n_digits)
    )]

    long[, display_color := ifelse(
      scaled_value > 0.6,
      "black",
      "white"
    )]
...
.scale_to_percentile_range <- function(x,
                                       lower_percentile = 0,
                                       upper_percentile = 100) {
  stopifnot(
    is.numeric(x),
  ...
  if (length(vals) == 0L) {
    stop("x contains no non-NA values.")
  }

  limits <- stats::quantile(
    vals,
    probs = c(lower_percentile, upper_percentile) / 100,
    na.rm = TRUE,
    names = FALSE
  )
  ...
  if (lower == upper) {
    stop(
      "The selected percentile limits are identical; ",
...
.scale_to_percentile_range <- function(x,
  ...
  if (length(lower_percentile) != 1L || is.na(lower_percentile) ||
    lower_percentile < 0 || lower_percentile >= 100) {
    stop(
      "lower_percentile must be a single value in [0, 100).",
      call. = FALSE
    )
  }
  if (length(upper_percentile) != 1L || is.na(upper_percentile) ||
    upper_percentile <= 0 || upper_percentile > 100) {
    stop(
      "upper_percentile must be a single value in (0, 100].",
      call. = FALSE
    )
  }
  ...
  if (lower == upper) {
    stop(
      "The selected percentile limits are identical; values cannot be scaled.",
      call. = FALSE
    )
  }
