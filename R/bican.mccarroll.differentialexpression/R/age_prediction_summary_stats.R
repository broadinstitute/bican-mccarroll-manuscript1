...
compute_age_error_metrics <- function(age_preds,
  ...
  stopifnot(is.data.frame(age_preds))
  req <- c(
    group_cols, "donor", age_col, pred_col,
    "num_nuclei", "num_umis", "num_features"
  )
  ...
  summarize_one_group <- function(d) {
    ...
    if (length(nf_u) > 1) {
      ...
      warning(
        "num_features not unique within group: ",
...
#'
#' @export
plot_residual_age_correlation_distributions <- function(corr_results,
                                                        fill_colors = c(
